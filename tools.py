...
def random_positive_sample(image_sample, pt, roi, ps):
    '''
    ...
    rotate_angle = 10
    image_sample_pad = np.pad(image_sample, ((ps//2,ps//2),(ps//2,ps//2),(0,0)),'minimum')
...
def visualize_patch(sample, pt, idx, sample_dir, name_tag = ''):
    ...
    img_path = sample_dir + str(idx) + '_' + str(pt[0]) + '_' + str(pt[1]) + name_tag + '.jpg'
