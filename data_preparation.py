...
pos_num_per_set = 20    # num of selected positive sample
neg_num_per_set = 10    # num of selected negative sample
mode = 'test'
# subj_list = [30001,30002]
if mode == 'train':
    subj_list =  [30001, 30002, 30004, 30007, 30008, 30010, 30011, 30012, 30013, 30015,
                30017, 30018, 30020, 30021, 30022, 30023]    # train
    dst_data_path = '../data/DSA_patch/train/'
if mode == 'test':
    subj_list = [30024,30026,30029,30031,30032] # test
    dst_data_path = '../data/DSA_patch/test/' # test

# subj_list = [30001, 30002, 30004, 30007, 30008, 30010, 30011, 30012, 30013, 30015,
            # 30017, 30018, 30020, 30021, 30022, 30023, 30024, 30026, 30029, 30031,
            # 30032, 30036, 30038, 30039, 30043, 30077, 30078, 30079, 30084, 30087, 30128]  # all
...
for subj in subj_list:
    ...
    for subj_series_name in subj_series_name_list:
        ...
        image = mean_normalize_volume(image)
            ...
            for i in range(len(pts)):
                ...
                for r in range(pos_num_per_set):
                    ...
                    if image_sample is None:
                        continue
                    positive_sample, positive_pt, position_ratio= random_positive_sample(image_sample, pts[i], roi, patch_size)
                    ...
                    # pdb.set_trace()
                    try:
                        visualize_patch(positive_sample, positive_pt, r, visualize_subj_series_path + 'pos/', name_tag = str(slc[i]))
        ...
        for i in range(1,image.shape[2]-1):
            ...
            for r in range(neg_num_per_set):
                ...
                if image_sample is None:
                    continue
                negative_sample, negative_pt, position_ratio= random_negative_sample(image_sample, pts, roi, patch_size)
                ...
                cnt_neg += 1
                try:
                    visualize_patch(negative_sample, negative_pt, r, visualize_subj_series_path+'neg/', name_tag = str(i))
